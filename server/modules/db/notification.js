...
module.exports = {
  getNotificationIdsByProperty: (prop, val) => {
    return getConnection().then(myDb => {
      return new Promise((resolve, reject) =>
        myDb.query(`select * from notificationList where ${prop} = ${mysql.escape(val)}`, (err, result) => {
          myDb.release()
          if (err) {
  ...
  deleteNotificationIdByProperty: (prop, val) => {
    return getConnection().then(myDb => {
      return new Promise((resolve, reject) =>
        myDb.query(`delete from notificationList where ${mysql.escapeId(prop)} = ${mysql.escape(val)};`, (err, result) => {
          myDb.release()
          if (err) {
  ...
  createNotificationId: (instance, options) => {
    const query = `INSERT INTO notificationList (
                ...
                ${mysql.escape(
                  crypto
                    .createHash('md5')
                    .update(JSON.stringify(options.subscription))
                    .digest('hex'),
                )},

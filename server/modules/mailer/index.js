...
module.exports = {
    sendDeadlineReminder(meal) {
        ...
        signupsAvailable
            ...
            .then((data) => {
                if (data.length) {
                    data.forEach(user => mail(deadlineReminder(user, meal), error.checkError(3, 'Error sending deadline reminder.'), user.name, 'deadlineReminder', meal.instance));
                }
    ...
    sendCreationNotice(instance, meal) {
        ...
        userDb.getUsersByProperty(instance, 'creationNotice', 1)
            .then((data) => {
                if (data.length) {
                    data.forEach(user => mail(meal.datefinder ? creationNotice_df(user, meal) : creationNotice(user, meal), error.checkError(3, 'Error sending creation notice.'), user.name, 'creationNotice', meal.instance));
                } else {

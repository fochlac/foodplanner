  ...
  switch (action.type) {
    ...
    case 'FINALIZE_PRICES':
      if (action.status === 'complete') {
        ...
        action.data.forEach(
          signupState => (newState[signupState.id] = Object.assign({}, newState[signupState.id], { price: signupState.price, paid: signupState.paid })),
        )
    ...
    case 'REFRESH':
      if (action.status === 'complete' && action.data.signups) {
        const signups = action.data.signups.reduce((acc, signup) => {
          acc[signup.id] = signup
          return acc
        }, {})

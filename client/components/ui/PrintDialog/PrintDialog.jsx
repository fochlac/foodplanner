  ...
  renderMeal(meal) {
    return (
      <tr key={meal.id}>
        ...
        <td>{meal.name}</td>
        <td><input type="checkbox" checked={this.state.selected.includes(meal.id)} onChange={this.setCheckbox.bind(this, meal.id, !this.state.selected.includes(meal.id))} /></td>
      </tr>
    );
  }
    ...
    return (
      <Dialog className="PrintDialog" closeOnBackdrop={true}>
        <div className="titlebar">
          ...
          <span className="fa fa-times push-right pointer" onClick={this.close}></span>
        </div>
        <div className="body">
          ...
          <table>
            ...
            <tbody>
              {
                this.props.meals.filter(meal => meal.time > startOfDay).sort((a,b) => (a.time - b.time)).map(this.renderMeal)
              }
            </tbody>
          </table>
        </div>
        <div className="foot">
          <button type="button" className="cancel" onClick={this.close}>Schließen</button>
          <button type="button" className="submit" onClick={this.print.bind(this)}>Drucken</button>
        </div>
      </Dialog>
    );
  }

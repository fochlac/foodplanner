...
export const signupUser = async function({ comment = 'Lorem ipsum dolor sit amet', options = [] }) {
  ...
  await Promise.all(
    options.map(async (option, index) => {
      ...
      switch (option.type) {
        ...
        case 'Auswahl':
          await optionElem
            .findElement(S.su.select)
            .findElement(S.su.signupOptionByName(option.value))
            .click()
          break
        case 'Ja-Nein':
...
export const createMeal = async function({
  ...
  const [nameInput, imageInput, descriptionInput, signupInput] = await Promise.all(
    [S.cm.name, S.cm.imageInput, S.cm.description, S.cm.signup].map(selector => this.driver.findElement(selector)),
  )
  ...
  await this.driver.wait(until.elementValueIs(descriptionInput, description), 1000)
  await this.driver.wait(until.elementValueIs(signupInput, signupLimit.toString()), 1000)
...
const addMealOption = async function(option, index) {
  ...
  if (option.values) {
    ...
    return this.driver.wait(until.elementCountIs(option.values.length, O.deleteAllValues, elem), 5000)
  } else {

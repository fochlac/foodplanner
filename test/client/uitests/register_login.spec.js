...
const checkLoggedin = async function(name) {
    ...
    return this.driver
      ...
      .then(text => expect(text).to.equal(name ? name : testname))
  },
...
describe('login area', () => {
  ...
  it('should be logged in after registering', async function() {
    ...
    await this.driver.createUser({ name: testname, mail: testmail })
  ...
  it('should be unable to register with same email', async function() {
    ...
    await this.driver.findElement(S.ld.name).sendKeys(testname)
    await this.driver.findElement(S.ld.mail).sendKeys(testmail)
  ...
  it('should be able to login with admin email', async function() {
    ...
    return this.checkLoggedin(this.username)
  })
})

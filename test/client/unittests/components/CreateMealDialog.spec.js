...
describe('CreateMealDialog', () => {
  ...
  it('should render all elements with preset values', () => {
    ...
    expect(wrapper.find('.deadline .timePicker').prop('defaultValue'), 'deadline hour select input not set correctly').to.equal(('00' + deadline.getHours()).slice(-2) + ':' + ((deadline.getMinutes() < 15 || deadline.getMinutes() > 45) ? '00' : '30'));
    expect(wrapper.find('.time .timePicker').prop('defaultValue'), 'time hour select not set correctly').to.equal(('00' + time.getHours()).slice(-2) + ':' + ((time.getMinutes() < 15 || time.getMinutes() > 45) ? '00' : '30'));
